...
def test_blocking_response():
    ...
    print(f"  status: {blocking_response.get('status')}")
    ...
    has_required_keys = all(
        key in blocking_response for key in ["success", "blocked", "message", "status"]
    )
    success_is_false = blocking_response.get("success") is False
    blocked_is_true = blocking_response.get("blocked") is True
    message_in_spanish = "factura" in message.lower() or "contexto" in message.lower()
    ...
    print_result(message_in_spanish, "Message in Spanish")
