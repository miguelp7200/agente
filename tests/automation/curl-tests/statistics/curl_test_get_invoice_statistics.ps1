# ===== SCRIPT CURL AUTOMATIZADO: Get Invoice Statistics =====
<#
.SYNOPSIS
    Test automatizado: Get Invoice Statistics

.DESCRIPTION
    Script generado automáticamente para validar: Obtener estadísticas generales del dataset de facturas
    
    Test Case: get_invoice_statistics
...
.EXAMPLE
    .\curl_test_get_invoice_statistics.ps1
    
.EXAMPLE  
    .\curl_test_get_invoice_statistics.ps1 -Environment Local -Verbose
#>
...
Write-Host "   TEST: Get Invoice Statistics" -ForegroundColor Magenta
...
# Variables del test
$sessionId = "auto-test-get_invoice_statistics-$(Get-Date -Format 'yyyyMMddHHmmss')"
$userId = "automation-test-user"
$appName = "gcp-invoice-agent-app"
$testQuery = "cuantas facturas hay?"
...
📈 RESULTADO FINAL:" -ForegroundColor Magenta
        ...
        $resultData = @{
            ...
            test_name = "Get Invoice Statistics"
        ...
        $resultFile = "../../results/result_get_invoice_statistics_$(Get-Date -Format 'yyyyMMddHHmmss').json"
...
Write-Success "Test Get Invoice Statistics completado!"
