# ===== SCRIPT CURL AUTOMATIZADO: Get Invoices With Pdf Info =====
<#
.SYNOPSIS
    Test automatizado: Get Invoices With Pdf Info

.DESCRIPTION
    Script generado automáticamente para validar: Obtener facturas con información completa de disponibilidad de PDFs
    
    Test Case: get_invoices_with_pdf_info
...
.EXAMPLE
    .\curl_test_get_invoices_with_pdf_info.ps1
    
.EXAMPLE  
    .\curl_test_get_invoices_with_pdf_info.ps1 -Environment Local -Verbose
#>
...
Write-Host "   TEST: Get Invoices With Pdf Info" -ForegroundColor Magenta
...
# Variables del test
$sessionId = "auto-test-get_invoices_with_pdf_info-$(Get-Date -Format 'yyyyMMddHHmmss')"
$userId = "automation-test-user"
$appName = "gcp-invoice-agent-app"
$testQuery = "dame facturas con información de qué PDFs tienen disponibles"
...
📈 RESULTADO FINAL:" -ForegroundColor Magenta
        ...
        $resultData = @{
            ...
            test_name = "Get Invoices With Pdf Info"
        ...
        $resultFile = "../../results/result_get_invoices_with_pdf_info_$(Get-Date -Format 'yyyyMMddHHmmss').json"
...
Write-Success "Test Get Invoices With Pdf Info completado!"
