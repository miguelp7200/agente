# ===== SCRIPT CURL AUTOMATIZADO: Get Cedible Sf Pdfs =====
<#
.SYNOPSIS
    Test automatizado: Get Cedible Sf Pdfs

.DESCRIPTION
    Script generado automáticamente para validar: Obtener todos los PDFs cedibles SF disponibles en el sistema
    
    Test Case: get_cedible_sf_pdfs
...
.EXAMPLE
    .\curl_test_get_cedible_sf_pdfs.ps1
    
.EXAMPLE  
    .\curl_test_get_cedible_sf_pdfs.ps1 -Environment Local -Verbose
#>
...
Write-Host "   TEST: Get Cedible Sf Pdfs" -ForegroundColor Magenta
...
# Variables del test
$sessionId = "auto-test-get_cedible_sf_pdfs-$(Get-Date -Format 'yyyyMMddHHmmss')"
$userId = "automation-test-user"
$appName = "gcp-invoice-agent-app"
$testQuery = "dame todas las facturas que tienen PDF cedible SF"
...
📈 RESULTADO FINAL:" -ForegroundColor Magenta
        ...
        # Guardar resultado
        $resultData = @{
            test_case = "get_cedible_sf_pdfs"
            test_name = "Get Cedible Sf Pdfs"
        ...
        $resultFile = "../../results/result_get_cedible_sf_pdfs_$(Get-Date -Format 'yyyyMMddHHmmss').json"
...
Write-Success "Test Get Cedible Sf Pdfs completado!"
