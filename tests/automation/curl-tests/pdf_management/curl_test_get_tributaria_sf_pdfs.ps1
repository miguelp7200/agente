# ===== SCRIPT CURL AUTOMATIZADO: Get Tributaria Sf Pdfs =====
<#
.SYNOPSIS
    Test automatizado: Get Tributaria Sf Pdfs

.DESCRIPTION
    Script generado automáticamente para validar: Obtener todos los PDFs tributarias SF disponibles en el sistema
    
    Test Case: get_tributaria_sf_pdfs
...
.EXAMPLE
    .\curl_test_get_tributaria_sf_pdfs.ps1
    
.EXAMPLE  
    .\curl_test_get_tributaria_sf_pdfs.ps1 -Environment Local -Verbose
#>
...
Write-Host "   TEST: Get Tributaria Sf Pdfs" -ForegroundColor Magenta
...
# Variables del test
$sessionId = "auto-test-get_tributaria_sf_pdfs-$(Get-Date -Format 'yyyyMMddHHmmss')"
$userId = "automation-test-user"
$appName = "gcp-invoice-agent-app"
$testQuery = "dame todas las facturas que tienen PDF tributaria SF"
...
📈 RESULTADO FINAL:" -ForegroundColor Magenta
        ...
        # Guardar resultado
        $resultData = @{
            test_case = "get_tributaria_sf_pdfs"
            test_name = "Get Tributaria Sf Pdfs"
        ...
        $resultFile = "../../results/result_get_tributaria_sf_pdfs_$(Get-Date -Format 'yyyyMMddHHmmss').json"
...
Write-Success "Test Get Tributaria Sf Pdfs completado!"
