...
class TestMultipleReferencesExtraction:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("Facturas 0011817764 y 0011817770", ["0011817764", "0011817770"]),
            # Mixed separators
            (
...
class TestExpectedPDFCounts:
    ...
    @pytest.mark.parametrize(
        "pdf_type,variant,expected_count",
        [
    ...
    def test_pdf_count_with_filter(
        self, pdf_type: str, variant: str, expected_count: int
    ):
    ...
    def test_original_bug_scenario(self):
        ...
        query = (
            "Busca facturas tributarias referencias 0011817764, 0011817770, 0011817773"
        )
...
class TestSystemPromptInstructions:
    ...
    def test_system_prompt_has_pdf_filtering_section(self):
        ...
        assert (
            "PDF TYPE FILTERING" in content
        ), "System prompt missing 'PDF TYPE FILTERING' section"
        assert (
            "tributaria_only" in content
        ), "System prompt missing 'tributaria_only' keyword"
        assert (
            "CF/SF VARIANT" in content or "cf" in content.lower()
    ...
    def test_tools_yaml_has_multiple_references_tool(self):
        ...
        assert (
            "search_invoices_by_multiple_references" in content
        ), "tools_updated.yaml missing 'search_invoices_by_multiple_references' tool"
        assert "reference_list" in content, "Tool missing 'reference_list' parameter"
        assert "pdf_type" in content, "Tool missing 'pdf_type' parameter"
        assert "pdf_variant" in content, "Tool missing 'pdf_variant' parameter"


class TestInvoiceFilterPdfPaths:
    """Test the Invoice.filter_pdf_paths() method."""

    @pytest.fixture
    def sample_invoice(self):
        """Create a sample invoice with all PDF types."""
        ...
        return Invoice(
            ...
            nombre="Test Company",
            pdf_paths={
                "Copia_Tributaria_cf": "gs://bucket/trib_cf.pdf",
                "Copia_Tributaria_sf": "gs://bucket/trib_sf.pdf",
                "Copia_Cedible_cf": "gs://bucket/ced_cf.pdf",
                "Copia_Cedible_sf": "gs://bucket/ced_sf.pdf",
                "Doc_Termico": "gs://bucket/termico.pdf",
            },
        )

    def test_filter_tributaria_only_cf(self, sample_invoice):
        """Filter should return only Tributaria CF."""
        filtered = sample_invoice.filter_pdf_paths("tributaria_only", "cf")
        assert filtered == {"Copia_Tributaria_cf": "gs://bucket/trib_cf.pdf"}

    def test_filter_tributaria_only_sf(self, sample_invoice):
        """Filter should return only Tributaria SF."""
        filtered = sample_invoice.filter_pdf_paths("tributaria_only", "sf")
        assert filtered == {"Copia_Tributaria_sf": "gs://bucket/trib_sf.pdf"}

    def test_filter_cedible_only_cf(self, sample_invoice):
        """Filter should return only Cedible CF."""
        filtered = sample_invoice.filter_pdf_paths("cedible_only", "cf")
        assert filtered == {"Copia_Cedible_cf": "gs://bucket/ced_cf.pdf"}

    def test_filter_termico_only(self, sample_invoice):
        """Filter should return only Doc Termico."""
        filtered = sample_invoice.filter_pdf_paths("termico_only", "cf")
        assert filtered == {"Doc_Termico": "gs://bucket/termico.pdf"}

    def test_filter_both_cf(self, sample_invoice):
        """Filter should return Tributaria CF + Cedible CF."""
        filtered = sample_invoice.filter_pdf_paths("both", "cf")
        assert filtered == {
            "Copia_Tributaria_cf": "gs://bucket/trib_cf.pdf",
            "Copia_Cedible_cf": "gs://bucket/ced_cf.pdf",
        }

    def test_filter_both_variants(self, sample_invoice):
        """Filter should return all Tributaria and Cedible variants."""
        filtered = sample_invoice.filter_pdf_paths("both", "both")
        assert len(filtered) == 4
        assert "Copia_Tributaria_cf" in filtered
        assert "Copia_Tributaria_sf" in filtered
        assert "Copia_Cedible_cf" in filtered
        assert "Copia_Cedible_sf" in filtered
        assert "Doc_Termico" not in filtered

    def test_filter_all(self, sample_invoice):
        """Filter 'all' should return all PDFs."""
        filtered = sample_invoice.filter_pdf_paths("all", "cf")
        assert len(filtered) == 5

    def test_default_is_both_cf(self, sample_invoice):
        """Default filter should be both + CF."""
        filtered = sample_invoice.filter_pdf_paths()
        assert filtered == {
            "Copia_Tributaria_cf": "gs://bucket/trib_cf.pdf",
            "Copia_Cedible_cf": "gs://bucket/ced_cf.pdf",
        }
