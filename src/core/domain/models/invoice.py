...
class Invoice:
    ...
    @classmethod
    def from_bigquery_row(
        cls, row: Dict[str, Any], field_mapping: Optional[Dict[str, str]] = None
    ) -> "Invoice":
        """
        ...
        return cls(
            ...
            factura_referencia=row.get(
                field_mapping.get("factura_referencia", "Factura_Referencia")
            ),
            detalles_factura=row.get(
                field_mapping.get("detalles_items", "DetallesFactura")
            ),
            pdf_paths=pdf_paths,
    ...
    def __repr__(self) -> str:
        return (
            f"Invoice(factura={self.factura!r}, rut={self.rut!r}, "
