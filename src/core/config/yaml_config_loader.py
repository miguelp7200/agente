...
class ConfigLoader:
    ...
    def __init__(self, config_path: Optional[Path] = None, service_name: Optional[str] = None):
        """
    ...
    def _load_env_vars(self):
        ...
        print("CONFIG No .env file found, using system environment only", file=sys.stderr)
    ...
    def _find_project_root(self) -> Path:
        ...
        for parent in [current] + list(current.parents):
            if (parent / "config.py").exists() or (parent / "requirements.txt").exists():
                return parent
    ...
    def _apply_env_overrides(self):
        ...
        if overrides_applied > 0:
            print(f"CONFIG Applied {overrides_applied} environment overrides", file=sys.stderr)
    ...
    def _apply_service_overrides(self):
        ...
        if service_override:
            ...
            print(f"CONFIG Applied service override: {self.service_name}", file=sys.stderr)
        else:
            # Use defaults if no specific override
            self._merged_config['service'] = services_config.get('defaults', {})
            print(f"CONFIG Using default service config (no override for {self.service_name})", file=sys.stderr)
    ...
    def _validate(self):
        ...
        try:
            ...
            if not read_project:
                ...
                errors.append(f"Invalid read project: {read_project}, expected: datalake-gasco")
            ...
            if not write_project:
                ...
                errors.append(f"Invalid write project: {write_project}, expected: agent-intelligence-gasco")
        ...
        try:
            ...
            if not (1 <= signed_url_hours <= 168):
                errors.append(f"pdf.signed_urls.expiration_hours must be 1-168, got: {signed_url_hours}")
                
            thinking_budget = self.get('vertex_ai.thinking.budget')
            if not (0 <= thinking_budget <= 8192):
                errors.append(f"vertex_ai.thinking.budget must be 0-8192, got: {thinking_budget}")
        except KeyError:
    ...
    def print_summary(self):
        ...
        print(f"  Invoices: {self.get_full_table_path('read', 'invoices')}", file=sys.stderr)
        print(f"  ZIP Packages: {self.get_full_table_path('write', 'zip_packages')}", file=sys.stderr)
        
        print(f"\n[PDF & ZIP]", file=sys.stderr)
        print(f"  Signed URL expiration: {self.get('pdf.signed_urls.expiration_hours')}h", file=sys.stderr)
        print(f"  ZIP threshold: {self.get('pdf.zip.threshold')} invoices", file=sys.stderr)
        print(f"  Max PDF links: {self.get('display.max_pdf_links')}", file=sys.stderr)
        
        print(f"\n[FEATURES]", file=sys.stderr)
        print(f"  Robust Signed URLs: {self.get('features.use_robust_signed_urls')}", file=sys.stderr)
        print(f"  Thinking Mode: {self.get('vertex_ai.thinking.enabled')}", file=sys.stderr)
        print(f"  Legacy Architecture: {self.get('features.use_legacy_architecture')}", file=sys.stderr)
...
def reload_config(config_path: Optional[Path] = None, service_name: Optional[str] = None):
